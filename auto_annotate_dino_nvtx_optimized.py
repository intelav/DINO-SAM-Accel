...
torch.backends.cuda.matmul.allow_tf32 = True  # ✅ use TF32 Tensor Cores for matmul
torch.backends.cudnn.allow_tf32 = True  # ✅ use TF32 in cuDNN convolutions too
...
def preprocess_batch(batch):
    """Safely stack and preprocess image batch for DINO."""
    # Filter out invalid or mismatched shapes
    valid = [b for b in batch if b is not None and b.ndim == 3 and b.shape[-1] == 3]
    if len(valid) == 0:
        raise ValueError("[Error] preprocess_batch: all input images invalid or empty")

    # Ensure all have same H,W; auto-pad or resize if necessary
    shapes = {b.shape[:2] for b in valid}
    if len(shapes) > 1:
        # Resize all to the first shape
        target_h, target_w = valid[0].shape[:2]
        valid = [cv2.resize(b, (target_w, target_h), interpolation=cv2.INTER_LINEAR)
                 for b in valid]
        print(f"[WARN] Resized batch items to uniform shape {target_h}x{target_w}")

    tensor = torch.from_numpy(np.stack(valid)).permute(0, 3, 1, 2).pin_memory()
    tensor = tensor.to("cuda", non_blocking=True)
    tensor = dino_transform(tensor)
    return tensor


def extract_dino_embeddings_batch(patches, batch_size=512):
    all_feats = []
    preproc_stream = torch.cuda.Stream()  # ✅ dedicated stream for preprocessing

    next_tensor_batch = None
    num_batches = (len(patches) + batch_size - 1) // batch_size

    for i in range(num_batches):
        # Slice next batch of patches
        start = i * batch_size
        end = min(start + batch_size, len(patches))
        batch = patches[start:end]

        # 🚀 Launch preprocessing of the *next* batch on the preproc_stream
        with torch.cuda.stream(preproc_stream):
            next_tensor_batch = preprocess_batch(batch)

        # 🚧 Wait for current batch preprocessing to complete before running model
        torch.cuda.current_stream().wait_stream(preproc_stream)

        # Run model on this batch
        with torch.no_grad():
            with torch.cuda.amp.autocast(dtype=torch.float16):
                nvtx.range_push(f"dino_forward_batch_{i}")
                outputs = model(pixel_values=next_tensor_batch)
                nvtx.range_pop()
...
def extract_dino_features_all(img, stride_frac=0.5, batch_size=512):
    """
    Extract DINO embeddings for multiple patch sizes efficiently.

    Args:
        img (H, W, 3) uint8 numpy array
        stride_frac: fractional stride relative to patch size
        batch_size: number of patches per embedding batch

    Returns:
        windows: list of (x, y, w, h)
        feats: list of embedding tensors
        size_to_indices: dict mapping patch size to global embedding indices
    """
    t0 = time.time()
    ...
    # Gather all unique patch sizes from CLASS_PATCH_SIZES_TUNED
    unique_sizes = sorted({ps for sizes in CLASS_PATCH_SIZES_TUNED.values() for ps in sizes})

    print(f"[Stage A] Processing patch sizes: {unique_sizes}")

    for ps in unique_sizes:
        stride = int(ps * stride_frac)
        patches_ps, windows_ps = [], []

        # --- Extract patches for this size ---
        for y in range(0, H - ps + 1, stride):
            for x in range(0, W - ps + 1, stride):
                crop = img[y:y + ps, x:x + ps, :]
                patches_ps.append(crop)
                windows_ps.append((x, y, ps, ps))

        if not patches_ps:
            continue

        # --- Embed patches for this size ---
        t_size0 = time.time()
        feats_ps = extract_dino_embeddings_batch(patches_ps, batch_size=batch_size)
        t_size1 = time.time()

        start_idx = len(windows)
        end_idx = start_idx + len(patches_ps)
        size_to_indices[ps] = list(range(start_idx, end_idx))

        windows.extend(windows_ps)
        feats_list.append(feats_ps)

        print(f"   • {ps}×{ps}: {len(patches_ps)} patches embedded in {t_size1 - t_size0:.2f}s")

    # Concatenate all features
    feats = torch.cat(feats_list, dim=0) if feats_list else torch.empty((0, model.config.hidden_size), device="cuda")

    t1 = time.time()
    print(f"[Stage A] DINO embedding finished in {t1 - t0:.2f} sec (batch_size={batch_size})")
...
def detect_candidates_precomputed(
    ...
    while min(current_thr.values()) >= 0.25 and len(candidates) < MAX_TOTAL and attempt < MAX_ATTEMPTS:
        new_candidates = []
...
def batch_embed_rois(rois, batch_size=64):
    ...
    """
    """Embed list of ROI crops safely."""
    rois = [r for r in rois if r is not None and r.size > 0 and r.ndim == 3]
    if not rois:
        print("[WARN] batch_embed_rois: no valid ROIs, returning empty tensor")
...
def main(args=None):
    ...
    if args.use_masked_feats and args.sam2 and args.sam_checkpoint:
        ...
        def masks_from_boxes_local(image, boxes, sam_predictor, sam_max_masks=3, chunk_size=16):
            ...
            for i in range(0, len(sam_boxes), chunk_size):
                ...
                nvtx.range_push("sam_predictor_predict")
    ...
    if args.review_only:
        ...
        if os.path.exists(meta_path) and os.path.exists(feats_path):
            ...
            nvtx.range_push("extract_dino_features")
